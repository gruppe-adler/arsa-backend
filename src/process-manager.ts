export class ProcessManager {
    ...
    constructor(cwd: string, executeable: string, args: string[]) {
        ...
        const command = new Deno.Command('sh', { cwd: cwd, args: shArgs, stdout: 'null', stderr: 'null' });

        // spawning the process
        this.childProcess = command.spawn();

        this.childProcess.status.then((status) => console.log(`Launched process finished with success status: ${status.success}`));
    }
}